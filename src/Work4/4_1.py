...
def in_shadow(p, light_pos):
    ...
    max_t = (light_pos - p).norm() - 0.001

    # 阴影射线起点轻微偏移，防止自遮挡
    ro = p + light_dir * 0.001

    t_sph, _ = intersect_sphere(ro, light_dir, ti.Vector([-1.2, -0.2, 0.0]), 1.2)
    t_cone, _ = intersect_cone(ro, light_dir, ti.Vector([1.2, 1.2, 0.0]), -1.4, 1.2)

    return (t_sph > 0 and t_sph < max_t) or (t_cone > 0 and t_cone < max_t)
...
def render():
    for i, j in pixels:
        ...
        if min_t < 1e9:
            ...
            # 光源设置
            light_pos = ti.Vector([4.0, 3.0, 2.0])
            ...
            # 硬阴影逻辑
            if in_shadow(p, light_pos):
                color = ambient  # 阴影中：只保留环境光
            else:
